...
async def short_redirect(short_code: str, request: Request):
    ...
    await ClicksDAO.add_click(
        link.id, request.client.host, request.headers.get("user-agent")
    )
...
async def delete_your_link(
    link_id: int, current_user: User = Depends(get_current_user)
):
