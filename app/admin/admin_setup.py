...
class LinkAdmin(ModelView, model=Link):
    column_list = [Link.id, Link.original_url, Link.short_code, Link.user_id, Link.is_active, Link.clicks_count, Link.created_at]

class ClickAdmin(ModelView, model=Click):
    column_list = [Click.id, Click.link_id, Click.ip_address, Click.user_agent, Click.clicked_at]
